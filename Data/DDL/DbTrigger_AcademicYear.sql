...
BEGIN
    ...
    FROM deleted d
        ...
        ON c.ContactId = d.AcademicYearId
END
end
GO
...
as
begin
if update(PersonID) or update(FirstName) or update(LastName) or update(SortName) or update(Phone) or update(Mobile) or update(Email) or update(AdressId) or  update(IsDeleted) or exists (select * from deleted)
BEGIN
    UPDATE a
        set a.AUTO_UPDATE_COUNT = a.AUTO_UPDATE_COUNT + 1, a.AUTO_TIME_UPDATE = GETDATE()
    FROM deleted d
...
IF EXISTS (select * from sysobjects where name like '%TG_StudentIEOM%')
DROP TRIGGER TG_StudentIEOM_Update
GO
Create trigger TG_StudentIEOM_Update
on StudentIEOM after insert, update
as
begin
if update(StudentIEOMId) or update(AcademicYearId) or update(StudentId) or update(IOEMId) or update(IsDeleted) or exists (select * from deleted)
BEGIN
    UPDATE a
        set a.AUTO_UPDATE_COUNT = a.AUTO_UPDATE_COUNT + 1, a.AUTO_TIME_UPDATE = GETDATE()
    FROM deleted d
        INNER JOIN StudentIEOM a
        ON a.StudentIEOMId = d.StudentIEOMId
END 
end
GO
...
IF EXISTS (select * from sysobjects where name like '%TG_Teacher%')
...
as
begin
    if update(TeacherPreferenceId) or update(Name) or update(Preference) or update(Description) or exists (select *
        from deleted)
BEGIN
        UPDATE tp
...
as
begin
    if update(TeacherCoordinationRoleInterestId) or update(Name) or update(AssignmentPercentage) or exists (select *
        from deleted)
BEGIN
        UPDATE tc
        set a.AUTO_UPDATE_COUNT = tc.AUTO_UPDATE_COUNT + 1, tc.AUTO_TIME_UPDATE = GETDATE()
    FROM deleted d
...
BEGIN
        UPDATE tap
        set tapi.AUTO_UPDATE_COUNT = tap.AUTO_UPDATE_COUNT + 1, tap.AUTO_TIME_UPDATE = GETDATE()
    FROM deleted d
            INNER JOIN TeacherAssignmentPercentageInterest tapi
            ON tap.TeacherAssignmentPercentageInterestId = d.TeacherAssignmentPercentageInterestId
    END
end
GO
